        ...
        if self.label.text() != "Path":
            ...
            if folder_path == "":
                ...
                for file in self.somefiles:
                    ...
                    # Create a list of sizes for the ICO files
                    sizes = [(128, 128), (256, 256), (512, 512), (1024, 1024), (96, 96)]
                    # Create a list to store the ICO images
                    ico_images = []
                    # Create ICO images for each size
                    for size in sizes:
                        ico_image = png_image.resize(size).convert("RGBA")
                        ico_images.append(ico_image)
                    # Save the ICO images
                    ico_image.save(f'{self.convert_slashes(folder_path)}\\{self.get_filename(file)}.ico', sizes=sizes)
                ...
                pyautogui.alert("已完成ICO制作\nThe ICO production task has been completed")  # 提示音
