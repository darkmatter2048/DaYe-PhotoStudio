...
"}")
    ...
    def start_working(self):
        if self.label.text() != "Path":
            ...
            video_path = os.path.join(f"{folder_path}", video_name)
            #video_path = video_name

            # 获取图片文件夹中的所有图片文件名
            images = self.somefiles

            # 获取第一张图片的尺寸
            frame = cv2.imread(images[0])
            height, width, layers = frame.shape

            # 使用 VideoWriter 对象创建视频
            ...
            pyautogui.alert("已完成视频合成\nThe Compositing video task has been completed")  # 提示音
            #os.startfile(folder_path)
            self.open_and_select_file(video_path)
