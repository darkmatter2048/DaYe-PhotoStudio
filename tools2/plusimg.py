    ...
    def start_working(self):
        if self.label.text() != "Path":
            ...
            if folder_path == "":
                ...
                images = [Image.open(path) for path in self.somefiles]
                ...
                # 保存拼接后的图片
                new_image.save(f"{self.convert_slashes(folder_path)}\\result.jpg")

                if self.ifaudio == "True":
                    self.play_sound(self.sound_file)
                pyautogui.alert("已完成图片拼接\nThe Image stitching task has been completed")  # 提示音
                #os.startfile(folder_path)
                self.open_and_select_file(f"{self.convert_slashes(folder_path)}\\result.jpg")
